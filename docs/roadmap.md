...
- 单测 +1 套件（test_frustum）：简单笛卡尔几何（fov90/aspect1 的半宽=深度）、
  近平面背后剔除、ECEF 相机下目标区在内/地球对侧在外。32 套件全绿（截至 34）。
...
- 单测 +2（test_terrain_lod_selector）：窄视场剪掉更多瓦（细阈值下 >0 且 < 宽视场）、
  朝天视锥（带近平面）兴趣窗内全剪 → 空。32 套件全绿零告警（截至 34）。
...
| A3.1 ✅ | **五固定机位（判据文档精确参数）真 DEM 截图集** | M-near(6瓦/335色) M-mid(42瓦/710色) M-graze(72瓦/640色) M-high(30瓦/594色) M-coarse(48瓦/536色)；docs/assets/station1..5.png + terrain.md 固定机位截图集表；观感待用户 |
| A3.0 ✅ | 真 DEM 默认源 + 三机位截图 | DEM 有资产即默认启用；M-near(3km/L13,6瓦) distinct=471、M-mid(15km/L12,48瓦) distinct=390、M-graze(8km/回退矩形,72瓦) distinct=348；截图 docs/assets/a3_realDEM_{Mnear,Mmid,Mgraze}*.png |
| A2.5 ✅ | **真实 DEM（terrarium 资产）模式**：559 瓦 z10–13（重庆缙云山，28MB assets）；DemAssetSource(AAssetManager)→PNG→Terrarium 解码(256²)→分带选择(alt→L10..13)→网格33段 | `debug.mapc.dem=1`；M-mid 15km/L12: 48瓦·10.4万三角形；M-near 3km/L13: 6瓦；截图 docs/assets/a3_realDEM_*.png（distinct 390/471，山脊沟谷连续明暗） |
...
> **最终状态核验（2026-09-08）**：最新 HEAD 全新 clone → host 构建 34/34 全绿

> distinct 色数为机读代理（BMP 采样步长 8–10 px，跨图比较仅看量级）；观感以全尺寸图为准。
