...
### B4 数值取证入账（2026-09-09，跨级 T-顶点裂缝度量）

- `SeamAudit::auditCrossLevelTVertexGap`：粗瓦边弦折线 vs 子瓦（z+1）边界节点最近
  ECEF 距离统计。语义注：跨级度量 >0 是**常态**（弦垂不可能为 0），与同级审计
  （理想 ≈0）口径不同；修复方向 = 子瓦边界吸附到粗弦（B4 remap/边 LUT），
  吸附量 = 本度量。
- 实测量级（test_cross_level_tvertex，z12 粗瓦 + z13 子瓦 fixture）：强地形曲率
  （K=2e8/rad²，弦垂 ~30m/整边）下粗瓦 2 段/边 → 裂缝 ~7m；8 段 → ~1.1m；
