...
| **S4** | **影像系统** | 无影像 provider（XYZ/WMTS/…）、无纹理上传预算 | ★★ 影像（阶段 3/4） | 复用 ITileBytesSource + URL 模板。**L1/L2 已落**：退化决议 + ImageryTileSource（装配链，host）+ PngToRgba8 + demo 每瓦真实影像纹理
（共享瓦片缓存单次下载；42/42 瓦无 miss，ledger upBytes 55.5MB，截图 realtex_Mmid.png）——
S4→渲染全链出图打通；下一步换真实影像源内容/图层接入 |
