...
**60 个 gtest 套件全绿零告警**（2026-09-09 复核：native 从零 54/54、全新 clone 54/54；此后 L2/L3 续增
至 60——渲染抽象/位移属性通道/引擎相机制/手势识别器/图层栈/矢量切片各+1 批）。
...
平台无关触摸流，7 用例）/图层栈（scene/LayerStack：顺序/开关/透明度/差分，host 5 用例）/矢量最小切片（vector/
VectorGrounding：GeoJSON 点解码+贴地+样式键，host 5 用例）/渲染抽象（IRenderDevice 纹理+UV+DrawList 设备验证）/PngToRgba8/RGBA alpha。
真实内容三层同屏：
...
- 引擎相机制（L3）：`debug.mapc.nav`（1=手势走引擎 MapCameraSystem：惯性滑行/贴地防护/flyTo/
  ...
  修复每重建解码+泄漏，hgt 模式同走持久映射）；磁盘缓存层（logcat `S2 disk`：冷启
  write=42 → warm 重启 hit=42 write=0，零网络 ~3s 出帧）。
